...
def hasAds(msg):
	links = ["http://","https://",".ir",'io',".com",".org",".net",".me"]
	for i in links:
		if i in msg:
			return True
			...
			try:
				if msg["type"]=="Text" and not msg.get("message_id") in answered:
					if not sleeped:
						...
						elif msg.get("text")== "محمد هکر؟" and msg.get("author_object_guid") in admins      :
								bot.sendMessage(target,'سلام جان؟',message_id=msg.get('message_id'))
						...
						elif msg.get('text').strtswith("محمد کدر؟") or msg.get('text').startswith("mohammad_coder?"):
							try:
								bot.sendMessage(target,"سلام بله ایشون مدیره گپه یا ادمین" , message_id=msg.get("message_id"))
							except:
								print("err M")	
